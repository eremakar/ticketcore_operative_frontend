...
import { BinaryFeatureTypes, binaryToFeatureForAccesor } from '../atoms/geojson-binary'
import {
  POINT_LAYER,
  LINE_LAYER,
  POLYGON_LAYER,
...
export interface SchemeGeometry extends Feature {
...
export type SchemeBasicLayerProps<DataT extends SchemeGeometry = SchemeGeometry> =
  _SchemeBasicLayerProps<DataT> & CompositeLayerProps<DataT>

/** Properties added by SchemeBasicLayer */
export type _SchemeBasicLayerProps<DataT extends SchemeGeometry = SchemeGeometry> = {
  /**
   ...
   */
  pointType?: string
} & _SchemeBasicLayerFillProps<DataT> &
  _SchemeBasicLayerStrokeProps<DataT> &
  _SchemeBasicLayer3DProps<DataT> &
  _SchemeBasicLayerPointCircleProps<DataT> &
  _SchemeBasicLayerIconPointProps<DataT> &
  _SchemeBasicLayerTextPointProps<DataT>

/** SchemeBasicLayer fill options. */
type _SchemeBasicLayerFillProps<DataT> = {
  /**
...
/** SchemeBasicLayer stroke options. */
type _SchemeBasicLayerStrokeProps<DataT> = {
  /**
...
/** SchemeBasicLayer 3D options. */
type _SchemeBasicLayer3DProps<DataT> = {
  /**
  ...
  /**
   * Material settings for lighting effect. Applies to extruded polygons.
   *
   * @default true
...
/** SchemeBasicLayer Properties forwarded to `ScatterPlotLayer` if `pointType` is `'circle'` */
export type _SchemeBasicLayerPointCircleProps<DataT> = {
...
/** SchemeBasicLayer properties forwarded to `IconLayer` if `pointType` is `'icon'` */
type _SchemeBasicLayerIconPointProps<DataT> = {
  iconAtlas?: any
  iconMapping?: any
...
/** SchemeBasicLayer properties forwarded to `TextLayer` if `pointType` is `'text'` */
type _SchemeBasicLayerTextPointProps<DataT> = {
...
const defaultProps: DefaultProps<SchemeBasicLayerProps> = {
...
type SchemeBasicPickingInfo = PickingInfo & {
  featureType?: string | null
  info?: any
}

/** Render GeoJSON formatted data as polygons, lines and points (circles, icons and/or texts). */
export class SchemeBasicLayer<
  DataT extends SchemeGeometry = SchemeGeometry,
  ExtraProps = {}
> extends CompositeLayer<Required<SchemeBasicLayerProps<DataT>> & ExtraProps> {
  static layerName = 'SchemeBasicLayer'
  ...
  getPickingInfo(params): SchemeBasicPickingInfo {
    const info = super.getPickingInfo(params) as SchemeBasicPickingInfo
  ...
  _updateAutoHighlight(info: SchemeBasicPickingInfo): void {
